...
static ICM42670_Status_t RouteFsyncInterrupt(
  ...
  status = ICM42670_ReadReg(config, ICM42670_REG_INT_SOURCE0, &int1_value,
                            1U);
  if (status != ICM42670_OK) {
    return status;
  }

  status = ICM42670_ReadReg(config, ICM42670_REG_INT_SOURCE3, &int2_value,
                            1U);
  if (status != ICM42670_OK) {
    return status;
  }
  ...
  status = ICM42670_WriteReg8(config, ICM42670_REG_INT_SOURCE0, int1_value);
  if (status != ICM42670_OK) {
    return status;
  }

  return ICM42670_WriteReg8(config, ICM42670_REG_INT_SOURCE3, int2_value);
}
...
ICM42670_Enable_Fsync(const ICM42670_Config *config,
  ...
  status = ICM42670_UpdateMreg1Bits(
      config, ICM42670_MREG1_REG_TMST_CONFIG1,
  ...
  status = ICM42670_UpdateMreg1Bits(
      config, ICM42670_MREG1_REG_FSYNC_CONFIG,
...
ICM42670_Status_t ICM42670_Disable_Fsync(const ICM42670_Config *config) {
  ...
  status = ICM42670_UpdateMreg1Bits(
      config, ICM42670_MREG1_REG_TMST_CONFIG1, ICM42670_TMST_CONFIG1_FSYNC_EN,
      0U);
  if (status != ICM42670_OK) {
    return status;
  }
...
ICM42670_Status_t ICM42670_Read_Fsync(const ICM42670_Config *config,
  ...
  if (ICM42670_ReadReg(config, ICM42670_REG_INT_STATUS,
                       &fsync_data->raw_status, 1U) !=
      ICM42670_OK) {
    return ICM42670_ERROR;
  }

  if (ICM42670_ReadReg(config, ICM42670_REG_TMST_FSYNCH, raw, 2U) !=
      ICM42670_OK) {
    return ICM42670_ERROR;
  }
